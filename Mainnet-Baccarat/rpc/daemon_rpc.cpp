...
int rpc::getdReamBalance()  /// Gets players dReam balance
{
    ...
    if(curldReamBalanceCheck) {
        ...
        curl_easy_setopt(curldReamBalanceCheck, CURLOPT_VERBOSE, 0L);
...
int rpc::fetchScData()       /// Get SC variables
{
    ...
    if(curlFetch) {
        ...
        QByteArray br = fetchReadBuffer.c_str();
        QJsonDocument cbDoc = QJsonDocument::fromJson(br);
        QJsonObject cbObj = cbDoc.object();
        QJsonObject cbResults = cbObj["result"].toObject();
...
int MainWindow::searchHandData()   /// Search hands stored on SC
{
    ...
    if(curlSearchHandData) {
        ...
        curl_easy_setopt(curlSearchHandData, CURLOPT_VERBOSE, 0L);
        ...
        ui->playerXCardLabel->setPixmap(displayCards(playerX.toInt()));
        ui->playerYCardLabel->setPixmap(displayCards(playerY.toInt()));
        ui->playerZCardLabel->setPixmap(displayDrawCard(playerZ.toInt()));

        ui->bankerTotalSpinBox->setValue(bankerTotal.toDouble());
        ui->bankerXCardLabel->setPixmap(displayCards(bankerX.toInt()));
        ui->bankerYCardLabel->setPixmap(displayCards(bankerY.toInt()));
        ui->bankerZCardLabel->setPixmap(displayDrawCard(bankerZ.toInt()));
...
QPixmap MainWindow::displayCards(int card) /// Card display
{
    QPixmap pcardEquiv;
        switch(card){
...
QPixmap MainWindow::displayDrawCard(int card)
{
    QPixmap pcardEquiv;
        switch(card){
