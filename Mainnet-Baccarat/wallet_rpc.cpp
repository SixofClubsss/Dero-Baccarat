...
int MainWindow::sitDown(void)      /// When player connects wallet
{
    CURL *curlSitDown;
    CURLcode res;
...
int MainWindow::playerBet(void)     /// Bet on player
{
    CURL *curlPlayer;
    CURLcode res;
    string pReadBuffer;
    ...
    if(curlPlayer) {
      ...
      QJsonValue txid = cbResults.value("txid");
        ...
        if(txid.isString())
        {
            ...
            rpc::thisHand = ui->dsbTotalHandsPlayed->value()-6;
...
int MainWindow::bankerBet(void)     /// Bet on banker
{
    CURL *curlBanker;
    CURLcode res;
    string bReadBuffer;
...
int MainWindow::tieBet(void)            /// Bet on tie
{
    CURL *curlTie;
    CURLcode res;
    string tReadBuffer;
